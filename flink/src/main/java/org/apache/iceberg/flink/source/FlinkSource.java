...
public class FlinkSource {
  ...
  public abstract static class Builder {
    ...
    private RowDataTypeInfo outputTypeInfo;
    ...
    public FlinkInputFormat buildInputFormat() {
      ...
      outputTypeInfo = RowDataTypeInfo.of((RowType) (flinkProjectedSchema == null ? tableSchema : flinkProjectedSchema)
              .toRowDataType().getLogicalType());

      expectedSchema = icebergSchema;
      if (flinkProjectedSchema != null) {
        expectedSchema = FlinkSchemaUtil.convert(icebergSchema, flinkProjectedSchema);
      }
    ...
    DataStream<RowData> buildBoundedSource() {
    ...
    DataStream<RowData> buildStreamingSource() {
      ...
      String opName = String.format("Iceberg Table(%s) ", tableName);
      return env.addSource(function, opName + "Monitor").transform(opName + "Reader", outputTypeInfo, factory);
    }
  }
