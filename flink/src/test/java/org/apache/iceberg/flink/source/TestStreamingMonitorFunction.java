...
import org.apache.iceberg.TestAppendHelper;
...
public class TestStreamingMonitorFunction {
  ...
  private StreamingMonitorFunction create() {
    return new StreamingMonitorFunction(
        TableLoader.fromHadoopTable(location), SCHEMA, Collections.emptyList(),
        ScanOptions.builder().monitorInterval(Duration.ofMillis(100)).build());
  }

  @Test
  public void testCheckpointRestore() throws Exception {
    TestAppendHelper appender = new TestAppendHelper(table, FileFormat.AVRO, TEMPORARY_FOLDER);
